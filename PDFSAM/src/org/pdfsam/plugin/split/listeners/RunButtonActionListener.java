...
public class RunButtonActionListener extends AbstractRunButtonActionListener {
	...
	private static final Logger log = Logger
			.getLogger(RunButtonActionListener.class.getPackage().getName());

	private SplitMainGUI panel;
	private PDFSplit splitter;
	private PDFSplit3000 splitter3000;
	...
	public void pDFSplit(String f, String type) {

		if (type.equals("BURST")) {
			splitter = new PDFSplit();
			try {
				splitter.doIt(f);
			} catch (Exception e) {
			...
			splitter3000 = new PDFSplit3000(f);
			try {
				splitter3000.doIt();
			} catch (IOException e) {
				// TODO Auto-generated catch block
		...
		log.info("++++++++++++++++++++++++++++++++++++++++");
		log.info("Done, please check the output!");
		log.info("\n\n\n");
		// System.exit(0);
	}
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (WorkExecutor.getInstance().getRunningThreads() > 0
				|| panel.getSelectionPanel().isAdding()) {
			DialogUtility.showWarningAddingDocument(panel);
			return;
		}
		if (!panel.getSameAsSourceRadio().isSelected()
				&& StringUtils.isEmpty(panel.getDestinationFolderText()
						.getText())) {
			// DialogUtility.showWarningNoDestinationSelected(panel,
			// DialogUtility.DIRECTORY_DESTINATION);
			// return;
		}
		PdfSelectionTableItem[] items = panel.getSelectionPanel()
				.getTableRows();
		if (items == null || items.length != 1) {
			DialogUtility.showWarningNoDocsSelected(panel,
					DialogUtility.ONE_DOC);
			return;
		}
		...
		try {
			...
			log.info("splitType----------" + splitType);
			// args.add(f);
			pDFSplit(f, splitType);
			DialogUtility.showRunSuccessDialog(panel, "箱贴分解成功!");

			if ((item.getPassword()) != null
					&& (item.getPassword()).length() > 0) {
			...
			// if (panel.getSplitSizeCombo().isSelectedItem() &&
			// panel.getSplitSizeCombo().isValidSelectedItem()) {
			// args.add(Long.toString(panel.getSplitSizeCombo().getSelectedBytes()));
			// } else {
			// throw new
			// Exception(GettextResource.gettext(Configuration.getInstance().getI18nResourceBundle(),
			// "Invalid split size"));
			// }
			...
			// int chosenOpt =
			// DialogUtility.showConfirmOuputLocationDialog(panel,
			// suggestedDir);
			...
			// .equals(((StringItem)
			// panel.getVersionCombo().getSelectedItem()).getId())) {
			// StringItem minItem = panel.getVersionCombo().getMinItem();
			// String currentPdfVersion =
			// Character.toString(item.getPdfVersion());
			// if (minItem != null) {
			// if (Integer.parseInt(currentPdfVersion) <
			// Integer.parseInt(minItem.getId())) {
			// if (JOptionPane.YES_OPTION !=
			// DialogUtility.askForPdfVersionConfilct(panel, minItem
			// .getDescription())) {
			// return;
			// }
			// }
			...
			// WorkExecutor.getInstance().execute(new
			// WorkThread(myStringArray));

		} catch (Exception ex) {
			log.error(GettextResource.gettext(Configuration.getInstance()
					.getI18nResourceBundle(), "Error: "), ex);
